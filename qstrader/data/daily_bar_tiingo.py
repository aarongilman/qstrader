...
class TiingoDailyBarDataSource(object):
    """
    ...
    def __init__(self, asset_type, adjust_prices=True, symbols=None, tiingo_api_key=None, start_date=None, end_date=None):
    ...
    def _load_ticker_prices_into_df(self, ticker):
        """
        ...
        """
        if self.start_date:
            df = self.client.get_dataframe(ticker,
                                           startDate=self.start_date,
                                           endDate=self.end_date,
                                           frequency='daily')
        else:
            df = self.client.get_dataframe(ticker, frequency='daily')
        # Ensure all timestamps are set to UTC for consistency
        df = df.set_index(df.index.tz_convert(pytz.UTC))
        return df
    ...
    def _load_csvs_into_dfs(self):
        ...
        """
        print("Loading ticker price data into DataFrames...")

        asset_frames = {}
        for ticker in self.symbols:
            asset_symbol = 'EQ:%s' % ticker
            print("Loading price data for symbol '%s'..." % asset_symbol)
            csv_df = self._load_ticker_prices_into_df(ticker)
    ...
    def _convert_bar_frame_into_bid_ask_df(self, bar_df):
        ...
        if self.adjust_prices:
            if 'adjClose' not in bar_df.columns:
                raise ValueError(
            ...
            oc_df = bar_df.loc[:, ['adjOpen', 'adjClose']]
